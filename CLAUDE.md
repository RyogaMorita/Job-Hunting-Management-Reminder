...
- **APIキーは `.gitignore` で隠す** — `.env` ファイルにキーを書き、`.gitignore` に `.env` を追加すること
- シークレットはEASの `eas secret set KEY_NAME value` で管理する（`app.json` には書かない）

### 事故防止ルール

#### 直書き事故
- `app.json` / `App.tsx` / その他ソースコードに APIキー・トークン・パスワードを直書き**絶対禁止**
- コミット前に `git diff` でシークレットが含まれていないか確認すること
- もし誤ってコミットした場合は即座にキーを無効化・再発行する（gitから消すだけでは不十分）

#### ログ流出事故
- `console.log` にトークン・APIレスポンス・ユーザーデータを出力しない
- デバッグ用の `console.log` はコミット前に必ず削除する（フックが検出するが最終確認も行う）
- 通知ペイロード・AsyncStorageのキー名に機密情報を含めない

#### 課金事故
- EAS Buildの**無料枠（月次上限）を超えると課金が発生する**
  - ビルド前に https://expo.dev のダッシュボードで残り枠を確認する
  - 無料枠が切れたら新しいEASアカウントを作成して対応（`eas logout` → `eas login`）
- Google AdMob・Apple IAP の本番キーをテスト環境で使わない
- App Store Connect APIキーの権限は必要最小限に留める
